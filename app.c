...
THD_FUNCTION(ui_button_thread, arg) {
  ...
  while(TRUE){
    ...
    else if((DS4.share && !ds4_share_old_state)){
      evt.type = UI_INPUT_BUTTON;
      evt.data.button_state = UI_BUTTON_BACK;
      need_post = true;
    }
...
int app_init(void) {
  ...
  InitDriving();
  chThdCreateStatic(wa_ui_button_thread, sizeof(wa_ui_button_thread), LOWPRIO+1, ui_button_thread, NULL);
  return 0;
}
